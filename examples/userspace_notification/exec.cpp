...
int consume(void * ctx, void * data, size_t size)
{
...
int main(int argc, char ** argv)
{
    ...
    Skeleton skeleton;
    skeleton->bss->parent_pid = atoi(argv[1]);
    int fd = bpf_map__fd(skeleton->maps.events);
    auto rb = ring_buffer__new(fd, consume, NULL, NULL);
